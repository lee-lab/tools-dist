...
- BETA: 利用者を**メールアドレスで招待**します（2026-09-27 から。参加用リンクは使いません）。利用者には登録フォームで Meta アカウントのメールアドレスを出してもらい、Dashboard の Members → Invite users に入力します。未受諾は Invited タブに並びます。既定の上限は **200 人**、申請すれば **2500 人**まで引き上げられます
...
| BETA（一般の利用者） | **参加用リンクは無効**。メール招待のみ | 登録フォームの回答を見て Dashboard から招待します |

BETA でリンクを使わないのは、リンクの参加リクエストでは Meta のアカウント名しか見えず、事業内の誰かを判断できないためです（自動承認にすると、URL を知る誰もが入れてしまいます）。

- **リンクは発行から 90 日で失効します。** そのチャネルに新しいビルドを上げると期限が延びるため、リリース頻度の高い ALPHA は実質失効しません
- **現行の ALPHA の URL の唯一の情報源は site-doc のインストールページ**（`content.ja/getting-started/install.md` と `content.en/getting-started/install.md` の Meta Quest 3 節）です。URL がローテートしたら **site-doc だけを直してください**
...
まだチャネルに入っていない利用者には、ALPHA なら[参加用リンク](#quest-チャネルの参加用リンクmmdagent-ex-のみ)を、BETA なら登録フォームを案内します。リンクそのものは伝えず、**手順ページの URL だけ**を伝えれば足ります。
